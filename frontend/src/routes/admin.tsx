...
function AdminControlCenter() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const shops = await apiClient.admin.getShops().catch(() => []);
      return {
        shops: (shops ?? []) as any[],
        sales: [] as any[],
        services: [] as any[],
  ...
  const shopsList: ShopRow[] = useMemo(() => {
    ...
    return data.shops
      .map((s: any) => {
        const sales = data.sales.filter((r: any) => (r.shop_id ?? r.shopId) === s.id);
        const services = data.services.filter((r: any) => (r.shop_id ?? r.shopId) === s.id);
        const items = data.serviceItems.filter((r: any) => (r.shop_id ?? r.shopId) === s.id);
        const products = data.products.filter((r: any) => (r.shop_id ?? r.shopId) === s.id);
        return {
          ...s,
          salesTotal: sales.reduce((a: number, r: any) => a + Number(r.price ?? 0) * Number(r.qty ?? 1), 0),
          salesCount: s._count?.sales ?? sales.length,
          serviceTotal: items.reduce((a: number, r: any) => a + Number(r.price ?? 0), 0),
          serviceCount: s._count?.services ?? services.length,
          productCount: s._count?.products ?? products.length,
          lowStock: products.filter((p: any) => Number(p.qty) <= Number(p.low_stock_threshold ?? p.lowStockThreshold ?? 5)).length,
        };
      })
      .filter((s: any) =>
        !term
          ? true
          : [s.name, s.email, s.contact, s.gst, s.address].some((v: any) => (v ?? "").toLowerCase().includes(term)),
      );
  }, [data, q]);
  ...
  const filteredSales = useMemo(() => {
    if (!data) return [];
    const term = q.trim().toLowerCase();
    return data.sales.filter((s: any) => {
      if (!term) return true;
      const shopName = data.shops.find((shp: any) => shp.id === (s.shop_id ?? s.shopId))?.name ?? "";
      return [s.product_name ?? s.productName, s.customer_name ?? s.customerName, s.phone, s.source, shopName]
  ...
  const filteredProducts = useMemo(() => {
    ...
    return data.products.filter((p: any) => {
      if (!term) return true;
      const shopName = data.shops.find((shp: any) => shp.id === (p.shop_id ?? p.shopId))?.name ?? "";
      return [p.model, p.category, p.product_type ?? p.productType, shopName]
        .some((v: any) => (v ?? "").toLowerCase().includes(term));
    });
  }, [data, q]);
  ...
  const filteredServices = useMemo(() => {
    if (!data) return [];
    const term = q.trim().toLowerCase();
    return data.services.filter((svc: any) => {
      if (!term) return true;
      const shopName = data.shops.find((shp: any) => shp.id === (svc.shop_id ?? svc.shopId))?.name ?? "";
      const techName = data.technicians.find((t: any) => t.id === (svc.technician_id ?? svc.technicianId))?.name ?? "";
      return [svc.customer_name ?? svc.customerName, svc.phone, svc.service_type ?? svc.serviceType, shopName, techName]
        .some((v: any) => (v ?? "").toLowerCase().includes(term));
    });
  }, [data, q]);
  ...
  function openEditModal(shop: ShopRow) {
    ...
    setEditLogoUrl(shop.logo_url ?? shop.logoUrl ?? "");
  }
  ...
  async function handleSaveShopEdit() {
    ...
    try {
      await apiClient.shops.updateCurrent({
        name: editName.trim(),
    ...
    try {
      ...
      qc.setQueryData(["admin-master-data"], (oldData: any) => {
        ...
        return {
          ...
          sales: (oldData.sales ?? []).filter((s: any) => (s.shop_id ?? s.shopId) !== shopId),
          services: (oldData.services ?? []).filter((s: any) => (s.shop_id ?? s.shopId) !== shopId),
          serviceItems: (oldData.serviceItems ?? []).filter((s: any) => (s.shop_id ?? s.shopId) !== shopId),
          products: (oldData.products ?? []).filter((p: any) => (p.shop_id ?? p.shopId) !== shopId),
          technicians: (oldData.technicians ?? []).filter((t: any) => (t.shop_id ?? t.shopId) !== shopId),
        };
      });
  ...
  async function handleDeleteSale(saleId: string, productName: string) {
    ...
    try {
      await apiClient.sales.delete(saleId);
  ...
  const systemTotals = useMemo(() => {
    const salesSum = (data?.sales ?? []).reduce((a: number, r: any) => a + Number(r.price ?? 0) * Number(r.qty ?? 1), 0);
    const serviceItemsSum = (data?.serviceItems ?? []).reduce((a: number, r: any) => a + Number(r.price ?? 0), 0);
    return {
  ...
  return (
    <div className="aurora-bg min-h-screen p-4 lg:p-8 text-foreground">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Stat label="Global Sales Revenue" value={fmtMoney(systemTotals.totalSalesRevenue)} icon={ShoppingCart} color="text-cyan-600 dark:text-cyan-400" />
                  ...
                  <tbody>
                    {shopsList.map((s) => (
                      <tr key={s.id}>
                        ...
                        <Td>{s.created_at ? s.created_at.slice(0, 10) : (s.createdAt ? s.createdAt.slice(0, 10) : "—")}</Td>
              ...
              <Table>
                ...
                <tbody>
                  {filteredSales.map((sale: any) => {
                    const shop = data?.shops.find((shp: any) => shp.id === (sale.shop_id ?? sale.shopId));
                    return (
                      <tr key={sale.id}>
                        <Td className="font-semibold text-primary">{upper(shop?.name ?? "Unknown Shop")}</Td>
                        <Td>
                          <div className="font-medium">{sale.customer_name ?? sale.customerName}</div>
                          <div className="text-xs text-muted-foreground">{sale.phone ?? "—"}</div>
                        </Td>
                        <Td className="font-medium">{sale.product_name ?? sale.productName}</Td>
                        <Td>{sale.qty}</Td>
                        ...
                        <Td className="text-right">
                          <button
                            onClick={() => handleDeleteSale(sale.id, sale.product_name ?? sale.productName ?? "sale")}
              ...
              <Table>
                ...
                <tbody>
                  {filteredProducts.map((prod: any) => {
                    const shop = data?.shops.find((shp: any) => shp.id === (prod.shop_id ?? prod.shopId));
                    const isLow = Number(prod.qty) <= Number(prod.low_stock_threshold ?? prod.lowStockThreshold ?? 5);
                    return (
                      <tr key={prod.id}>
                        <Td className="font-semibold text-primary">{upper(shop?.name ?? "Unknown Shop")}</Td>
              ...
              <Table>
                ...
                <tbody>
                  {filteredServices.map((svc: any) => {
                    const shop = data?.shops.find((shp: any) => shp.id === (svc.shop_id ?? svc.shopId));
                    const tech = data?.technicians.find((t: any) => t.id === (svc.technician_id ?? svc.technicianId));
                    return (
                      <tr key={svc.id}>
                        <Td className="font-semibold text-primary">{upper(shop?.name ?? "Unknown Shop")}</Td>
                        <Td>
                          <div className="font-bold">{svc.customer_name ?? svc.customerName}</div>
                          <div className="text-xs text-muted-foreground">{svc.phone ?? "—"}</div>
                        </Td>
                        <Td>
                          <span className="font-semibold">{svc.service_type ?? svc.serviceType}</span>
                          {(svc.is_filter_change ?? svc.isFilterChange) && (
                        ...
                        <Td>{fmtDate(svc.service_date ?? svc.serviceDate)}</Td>
                        <Td className="text-amber-400 font-medium font-mono">
                          {fmtDate(svc.next_service_date ?? svc.nextServiceDate)}
                        </Td>
                      </tr>
                    );
                  })}
