...
function Dashboard() {
  const { data: shop } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await apiClient.shops.getCurrent();
      } catch {
        return null;
      }
    },
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [salesRows, productsRows, servicesRows, emisRows] = await Promise.all([
        apiClient.sales.list().catch(() => []),
        apiClient.products.list().catch(() => []),
        apiClient.services.list().catch(() => []),
        apiClient.emi.list().catch(() => []),
      ]);

      const remindersRows = servicesRows.filter((s: any) => {
        if (!s.isFilterChange && !s.is_filter_change) return false;
        const nextDate = s.nextServiceDate || s.next_service_date;
        if (!nextDate) return false;
        const dStr = typeof nextDate === "string" ? nextDate.slice(0, 10) : new Date(nextDate).toISOString().slice(0, 10);
        return dStr <= in30.toISOString().slice(0, 10);
      });

      const totalSalesAmt = (rows: any[]) => rows.reduce((s: number, r: any) => s + Number(r.price || 0) * Number(r.qty || 1), 0);
      const getSaleDateStr = (r: any) => {
        const d = r.saleDate || r.sale_date;
        if (!d) return "";
        return typeof d === "string" ? d.slice(0, 10) : new Date(d).toISOString().slice(0, 10);
      };
      const getServiceDateStr = (s: any) => {
        const d = s.serviceDate || s.service_date;
        if (!d) return "";
        return typeof d === "string" ? d.slice(0, 10) : new Date(d).toISOString().slice(0, 10);
      };

      const todaySalesAmt = totalSalesAmt(salesRows.filter((r: any) => getSaleDateStr(r) === todayStr));
      const monthSalesAmt = totalSalesAmt(salesRows.filter((r: any) => getSaleDateStr(r) >= monthStartStr));
      ...
      servicesRows.forEach((s: any) => {
        const dStr = getServiceDateStr(s);
        if (!dStr) return;
        const dObj = new Date(dStr);
        if (dObj.getFullYear() === year && dObj.getMonth() === month) {
          const day = dObj.getDate();
          const itemsList = s.serviceItems || s.service_items || [];
          const itemsTotal = itemsList.reduce((acc: number, item: any) => acc + Number(item.price || 0), 0);
      ...
      const todaySalesList = salesRows.filter((r: any) => getSaleDateStr(r) === todayStr).map((r: any) => ({
        id: r.id,
        type: r.source === "office" ? "Office" : "Sales",
        name: r.productName || r.product_name,
        customer: r.customerName || r.customer_name || "Direct Sale",
        amount: Number(r.price || 0) * Number(r.qty || 1),
      }));

      const todayServiceList = servicesRows.filter((s: any) => getServiceDateStr(s) === todayStr).map((s: any) => {
        const itemsList = s.serviceItems || s.service_items || [];
        const amt = itemsList.reduce((acc: number, item: any) => acc + Number(item.price || 0), 0);
        return {
          id: s.id,
          type: "Service",
          name: s.serviceType || s.service_type || "Service",
      ...
      const lowStock = productsRows.filter((p: any) => Number(p.qty) <= Number(p.low_stock_threshold ?? 2));
        ...
        <Card hover className="p-6">
          ...
          {stats?.reminders.length ? (
            <ul className="space-y-2.5 text-sm">
              {stats.reminders.slice(0, 5).map((s: any) => {
                ...
                return (
                  <li key={s.id} className="flex items-center justify-between rounded-xl glass p-3 uppercase-data border border-border/50 hover:border-primary/30 transition">
                    <div>
                      <div className="font-bold text-foreground">{cName}</div>
                      <div className="text-xs text-muted-foreground font-mono mt-0.5">Due {fmtDate(nextDate)}</div>
                    </div>
                    {s.phone && (
                      <a
                        href={waLink(s.phone, `Hello ${upper(cName)}, your RO filter change is due on ${fmtDate(nextDate)}. Please book a service.`)}
                        target="_blank"
                        rel="noreferrer"
