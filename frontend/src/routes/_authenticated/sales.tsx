...
export function SalesPage() {
  ...
  const { data: sales = [] } = useQuery({
    queryKey: ["sales"],
    queryFn: async () => await apiClient.sales.list(),
  });
  ...
  const showroomTotal = showroomSales.reduce((acc: number, s: any) => acc + Number(s.price) * Number(s.qty), 0);
  const officeTotal = officeSales.reduce((acc: number, s: any) => acc + Number(s.price) * Number(s.qty), 0);
...
function StockSaleForm({ onDone }: { onDone: () => void }) {
  const { data: products = [] } = useQuery({
    queryKey: ["products"],
    queryFn: async () => await apiClient.products.list(),
  });
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      await apiClient.sales.create({
        ...
        productName: String(selected.model).toUpperCase(),
        productType: selected.productType ?? selected.product_type ?? null,
        price: f.price || Number(selected.price),
        qty: f.qty,
        customerName: f.customer_name.trim().toUpperCase() || null,
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); submit.mutate(); }} className="grid grid-cols-1 gap-3 md:grid-cols-2">
      ...
      <Field label="Price (₹)"><Input type="number" step="0.01" min={0} value={f.price} onChange={(e: any) => setF({ ...f, price: Number(e.target.value) })} /></Field>
      <Field label={`Quantity${selected ? ` (max ${selected.qty})` : ""}`}><Input type="number" min={1} value={f.qty} onChange={(e: any) => setF({ ...f, qty: Number(e.target.value) })} /></Field>
      <Field label="Date"><Input type="date" value={f.sale_date} onChange={(e: any) => setF({ ...f, sale_date: e.target.value })} /></Field>
      <Field label="Customer name"><Input value={f.customer_name} onChange={(e: any) => setF({ ...f, customer_name: e.target.value })} className="uppercase-data" /></Field>
      <Field label="Phone"><Input maxLength={10} value={f.phone} onChange={(e: any) => setF({ ...f, phone: e.target.value.replace(/\D/g, "").slice(0, 10) })} placeholder="10-digit mobile number" className="font-mono tracking-wider" /></Field>
      <Field label="Place / Address"><Input value={f.address} onChange={(e: any) => setF({ ...f, address: e.target.value })} className="uppercase-data" /></Field>
...
function ManualSaleForm({ source, onDone }: { source: "manual" | "office"; onDone: () => void }) {
  ...
  const submit = useMutation({
    mutationFn: async () => {
      await apiClient.sales.create({
        source,
        productName: f.product_name.trim().toUpperCase(),
        productType: f.product_type.trim() || null,
        price: f.price,
        qty: f.qty,
        customerName: f.customer_name.trim().toUpperCase() || null,
        phone: f.phone.trim() || null,
  ...
  return (
    <form onSubmit={(e: any) => { e.preventDefault(); submit.mutate(); }} className="grid grid-cols-1 gap-3 md:grid-cols-2">
      <Field label="Product / Item name">
        <Input
          required
          value={f.product_name}
          onChange={(e: any) => setF({ ...f, product_name: e.target.value })}
      ...
      <Field label="Product type">
        <Input
          ...
          onChange={(e: any) => setF({ ...f, product_type: e.target.value })}
          placeholder="e.g. Commercial / Industrial / Spare"
        />
      </Field>
      <Field label="Price (₹)">
        <Input type="number" step="0.01" min={0} value={f.price} onChange={(e: any) => setF({ ...f, price: Number(e.target.value) })} />
      </Field>
      <Field label="Quantity">
        <Input type="number" min={1} value={f.qty} onChange={(e: any) => setF({ ...f, qty: Number(e.target.value) })} />
      </Field>
      <Field label="Customer / Company name">
        <Input
          value={f.customer_name}
          onChange={(e: any) => setF({ ...f, customer_name: e.target.value })}
          placeholder="Customer or Organization Name"
          className="uppercase-data"
        />
      </Field>
      <Field label="Phone number">
        <Input maxLength={10} value={f.phone} onChange={(e: any) => setF({ ...f, phone: e.target.value.replace(/\D/g, "").slice(0, 10) })} placeholder="10-digit mobile number" className="font-mono tracking-wider" />
      </Field>
      <Field label="Place / Address">
        <Input
          value={f.address}
          onChange={(e: any) => setF({ ...f, address: e.target.value })}
          placeholder="Installation site or office location"
          className="uppercase-data"
        />
      </Field>
      <Field label="Sale Date">
        <Input type="date" value={f.sale_date} onChange={(e: any) => setF({ ...f, sale_date: e.target.value })} />
      </Field>
      <div className="md:col-span-2 flex justify-end">
        <Button disabled={submit.isPending}>
