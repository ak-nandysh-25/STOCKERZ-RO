...
function StockPage() {
  ...
  const { data: products = [] } = useQuery({
    queryKey: ["products"],
    queryFn: async () => {
      return await apiClient.products.list();
    },
  });
  ...
  return (
    <div>
      ...
      {products.length === 0 ? (
        <Card>
          <Empty text="No products yet. Add your first item." />
        </Card>
      ) : (
        <Table>
          <thead>
            ...
            {products.map((p) => {
              ...
              return (
                <tr key={p.id} className="hover:bg-white/5">
                  ...
                  <Td>{p.category}</Td>
                  <Td>
                    <span className={isLow ? "inline-flex items-center gap-1 text-warning" : ""}>
                  ...
                  <Td className="text-right">
                    <button className="mr-2 rounded p-1.5 hover:bg-white/10" onClick={() => setModal(p)}>
...
function ProductModal({ open, product, onClose }: { open: boolean; product: Product | null; onClose: () => void }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (product) {
        await apiClient.products.update(product.id, payload);
      } else {
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Field label="Low stock alert">
            <Input
              ...
              onChange={(e) => setF({ ...f, lowStockThreshold: Number(e.target.value) })}
            />
          </Field>
        </div>
