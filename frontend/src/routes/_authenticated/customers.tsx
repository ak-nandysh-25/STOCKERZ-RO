...
function CustomersPage() {
  ...
  const customers = useMemo<Row[]>(() => {
    ...
    for (const s of data.sales) {
      ...
      const saleDate = s.saleDate ? (typeof s.saleDate === "string" ? s.saleDate.slice(0, 10) : new Date(s.saleDate).toISOString().slice(0, 10)) : (s.sale_date || "");
      if (!customerName && !s.phone) continue;

      const k = key(customerName, s.phone);
      const cur = map.get(k) ?? { key: k, name: customerName ?? "—", phone: s.phone, address: s.address, lastDate: saleDate, salesCount: 0, servicesCount: 0, totalSpent: 0 };
      cur.name = cur.name === "—" ? (customerName ?? "—") : cur.name;
      cur.phone = cur.phone ?? s.phone;
      cur.address = cur.address ?? s.address;
      cur.salesCount += 1;
      cur.totalSpent += Number(s.price) * Number(s.qty);
      if (saleDate > cur.lastDate) cur.lastDate = saleDate;
      map.set(k, cur);
    }

    for (const s of data.services) {
      const customerName = s.customerName || s.customer_name;
      const serviceDate = s.serviceDate ? (typeof s.serviceDate === "string" ? s.serviceDate.slice(0, 10) : new Date(s.serviceDate).toISOString().slice(0, 10)) : (s.service_date || "");
      const itemList = s.serviceItems || s.service_items || [];

      const k = key(customerName, s.phone);
      const cur = map.get(k) ?? { key: k, name: customerName, phone: s.phone, address: s.address, lastDate: serviceDate, salesCount: 0, servicesCount: 0, totalSpent: 0 };
      cur.phone = cur.phone ?? s.phone;
      cur.address = cur.address ?? s.address;
      cur.servicesCount += 1;
      cur.totalSpent += itemList.reduce((a: number, it: any) => a + Number(it.price), 0);
      if (serviceDate > cur.lastDate) cur.lastDate = serviceDate;
      map.set(k, cur);
    }
  ...
  if (active && data) {
    const sales = data.sales.filter((s: any) => {
      const customerName = s.customerName || s.customer_name;
      const k = (s.phone?.replace(/\D/g, "") || (customerName ?? "").trim().toLowerCase()) || "__unknown";
      return k === active.key;
    });
                ...
                <Card key={s.id} className="!p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1 uppercase-data">
                      ...
                      <div className="text-xs text-muted-foreground">{saleDate} • QTY {s.qty} • {s.source}</div>
                    </div>
        ...
        {services.length === 0 ? <Card><Empty text="No services" /></Card> : (
          <div className="space-y-2">
            {services.map((s: any) => {
              ...
              const itemList = s.serviceItems || s.service_items || [];
              const total = itemList.reduce((a: number, it: any) => a + Number(it.price), 0);
              return (
                <Card key={s.id} className="!p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1 uppercase-data">
                      <div className="truncate font-medium">{serviceType}</div>
                      <div className="text-xs text-muted-foreground">{fmtDate(serviceDate)} • {itemList.length} ITEMS {nextServiceDate ? `• NEXT ${fmtDate(nextServiceDate)}` : ""}</div>
                    </div>
                    <div className="text-right">
